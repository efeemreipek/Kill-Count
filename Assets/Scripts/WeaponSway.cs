...
public class WeaponSway : MonoBehaviour
{
    ...
    [Header("Jump and Fall Sway")]
    [SerializeField] private float jumpFallSwayAmount = 0.05f;
    [SerializeField] private float jumpFallSwaySpeed = 5f;
...
private void Update()
{
    ...
    {
        float verticalSpeed = playerController.GetMovementInput().y;
        jumpFallOffset = Vector3.up * jumpFallSwayAmount * Mathf.Sin(Time.time * jumpFallSwaySpeed) * (verticalSpeed != 0 ? verticalSpeed : 1);
    }

    weaponTransform.localPosition = Vector3.Lerp(
        weaponTransform.localPosition, 
        new Vector3(sway.x, sway.y, 0) * positionSwayMultiplier * Mathf.Deg2Rad + initialPosition + jumpFallOffset, 
        swayCurve.Evaluate(Time.deltaTime * swaySmooth)
    );
    weaponTransform.localRotation = Quaternion.Slerp(
        weaponTransform.localRotation, 
        initialRotation * Quaternion.Euler(Mathf.Rad2Deg * rotationSwayMultiplier * new Vector3(-sway.y, sway.x, 0)), 
        swayCurve.Evaluate(Time.deltaTime * swaySmooth)
    );
}
